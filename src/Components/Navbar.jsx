...
import {
  ...
  FaHome, FaInfoCircle, FaStethoscope, FaPhoneAlt, FaVideo, FaSignOutAlt
} from "react-icons/fa";
...
const HospilightNavbar = () => {
  ...
  const navLinks = [
    ...
    {
      name: "Products", dropdown: true, icon: <FaStethoscope />,
      items: [
        ...
        { name: "Derma Chair", path: "/product/derma-chair" },
  ...
  return (
    <>
      ...
      {/* ✅ Main Navbar */}
      <header className="sticky top-0 w-full z-50 bg-white shadow-xl border-b border-gray-200">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <img src={logo} alt="Hospilight Logo" className="w-16 md:w-20 object-contain" />
          ...
          <div className="flex gap-4 items-center">
            <div className="hidden md:block relative">
              <input type="text" placeholder="Search..." className="px-4 py-2 text-sm rounded-full border border-gray-300 shadow-inner focus:ring-2 focus:ring-[#38C172] outline-none transition" />
          ...
          {navLinks.map((link) =>
            link.dropdown ? (
              ...
              >
                ...
                <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-52 bg-white rounded-lg shadow-lg border border-gray-100 z-20 transition-all ${dropdownOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
                  {link.items.map((item) => (
                    <Link key={item.name} to={item.path} className="block px-4 py-2 text-[#3A8DFF] hover:bg-[#E6F3FF] hover:text-[#38C172] transition">
                      {item.name}
                    </Link>
                  ))}
                </div>
              ...
              <Link key={link.name} to={link.path} className="flex items-center gap-2 hover:text-[#38C172] transition">
                {link.icon} {link.name}
              </Link>
            )
          )}
        ...
        {/* ✅ Mobile Menu */}
        <div className={`fixed top-0 left-0 w-[85%] max-w-xs h-screen z-50 bg-white transform ${menuOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-500 ease-in-out shadow-2xl overflow-y-auto p-6`}>
          <ul className="space-y-5 text-[#3A8DFF] font-medium text-base">
            {navLinks.map((link) =>
              link.dropdown ? (
                <div key={link.name}>
                  <div onClick={toggleDropdown} className="flex justify-between items-center cursor-pointer hover:text-[#38C172]">
                    <span className="flex items-center gap-2">{link.icon} {link.name}</span>
                    <IoChevronDownOutline className={`transition-transform duration-300 ${dropdownOpen ? "rotate-180" : ""}`} />
                  </div>
                  {dropdownOpen && (
                    <ul className="pl-4 mt-2 space-y-2 text-gray-700 text-sm">
                      {link.items.map((item) => (
                        <li key={item.name}>
                          <Link to={item.path} onClick={() => setMenuOpen(false)} className="block hover:text-[#38C172]">
                            {item.name}
                          </Link>
                        </li>
                      ))}
                ...
                <li key={link.name}>
                  <Link to={link.path} onClick={() => setMenuOpen(false)} className="flex gap-2 items-center hover:text-[#38C172]">
                    {link.icon} {link.name}
                  </Link>
                </li>
              )
